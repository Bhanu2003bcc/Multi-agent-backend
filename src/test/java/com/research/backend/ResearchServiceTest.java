...
class ResearchServiceTest {
    ...
    @Mock ResearchJobRepository jobRepository;
    @Mock ResearchResultRepository resultRepository;
    @Mock AgentGatewayClient agentClient;
    @Mock CacheService cacheService;
    @Mock JobEventPublisher eventPublisher;
    @Mock ResearchMapper mapper;
    ...
    void submitJob_cacheMiss_returnsJobSubmittedResponse() {
        ...
        // In a real integration test this would run via Spring context
        JobSubmittedResponse response = researchService.submitJob(validRequest, "testuser");

        assertThat(response).isNotNull();
        verify(jobRepository, times(1)).save(any(ResearchJob.class));
    ...
    void submitJob_cacheHit_doesNotCallAgentClient() {
        com.research.backend.dto.response.ResearchResultResponse cachedResult =
                com.research.backend.dto.response.ResearchResultResponse.builder()
